...
df <- df %>%
  mutate(coach_exp_diff = coach_exp_start - coach_exp_start_1yr,
         ...
         win_pct_last2yr = case_when(!is.na(w_2yr) ~ (w_1yr + w_2yr) / (g_1yr + g_2yr),
                                     TRUE ~ win_pct_1yr),
         win_pct_last3yr = case_when(!is.na(w_3yr) ~ (w_1yr + w_2yr + w_3yr) / (g_1yr + g_2yr + g_3yr),
                                     !is.na(w_2yr) ~ win_pct_last2yr,
                                     TRUE ~ win_pct_1yr),
         proj_win_pct_last2yr = case_when(!is.na(pw_2yr) ~ (pw_1yr + pw_2yr) / (g_1yr + g_2yr),
                                          TRUE ~ proj_win_pct_1yr),
         proj_win_pct_last3yr = case_when(!is.na(pw_3yr) ~ (pw_1yr + pw_2yr + pw_3yr) / (g_1yr + g_2yr + g_3yr),
                                          !is.na(pw_2yr) ~ proj_win_pct_last2yr,
                                          TRUE ~ proj_win_pct_1yr),
